...
def get_post_direct_judge_list(query, instruct):
    return f"Question: {query}.\n\n The requirements for judging whether a passage has utility in answering the question are: The passage has utility in answering the question, meaning that the passage not only be relevant to the question, but also be useful in generating a correct, reasonable and perfect answer to the question. \n"+instruct
def get_direct_judge_list(question, instruct, passages, max_words_per_passage=150):
    ...
    for content in passages:
        ...
        if len(words) > max_words_per_passage:
            content = ' '.join(words[:max_words_per_passage])
...
def main(file_type, llm, tokenizer, instruct, types, sampling_params):
    ...
    path = './results/Qwen3-8B-no-thinking/'
    ...
    try:
        for sample in tqdm(all_data[begin:len(all_data)], desc="Filename: %s" % args.outfile):
            # 兼容不同的数据格式：passage/passages, labels/ground_truth_label
...
if __name__ == '__main__':
    ...
    tokenizer = transformers.AutoTokenizer.from_pretrained("./models/Qwen3-8B-GPTQ", trust_remote_code=True)
    llm = LLM(model="./models/Qwen3-8B-GPTQ",
        trust_remote_code=True,
        gpu_memory_utilization=0.85,  # 根据显存调整
        max_model_len=16384,
        enforce_eager=True
        )
    instruct = """
    ...
    """
    # GTI-NQ 的段落较长，暂跳过。如需要运行 GTI-NQ 的 k-sampling，可增大 max_model_len 或进一步截断段落
    # main("data/GTI-NQ.json", llm, tokenizer, instruct, "NQrandom-k-sampling-", sampling_params)    
    main("data/trec.json", llm, tokenizer, instruct, "trec-k-sampling-", sampling_params)
    main("data/webap.json", llm, tokenizer, instruct, "webap-k-sampling-", sampling_params)